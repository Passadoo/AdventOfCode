def Day3(data1, data2):
    firstLine = (0, 0)
    secondLine = (0, 0)
    firstLineVisited = set()
    bothVisited = set()

    #Fill firstLineVisited array
    for instruction in data1:
        if instruction[0] is "R":
            #Move to the right
            distance = int(instruction[1:])
            for i in range(1, distance + 1): # from 1 to distance
                firstLineVisited.add((firstLine[0] + i, firstLine[1]))
            firstLine = (firstLine[0] + distance, firstLine[1])
            ...
            distance = int(instruction[1:])
            for i in range(1, distance + 1):
                firstLineVisited.add((firstLine[0], firstLine[1] + i))
            firstLine = (firstLine[0], firstLine[1] + distance)
    ...
    #Fill secondLineVisited array
    for instruction in data2:

        tempShortestDistance = 5000000
        for item in bothVisited:
            val = abs(item[0]) + abs(item[1])
            if val < tempShortestDistance:
                tempShortestDistance = val

        if instruction[0] is "R":
            #Move to the right
            distance = int(instruction[1:])
            for i in range(1, distance + 1): # from 1 to distance
                if abs(secondLine[0] + i) + abs(secondLine[1]) < tempShortestDistance:
                    if (secondLine[0] + i, secondLine[1]) in firstLineVisited:
                        bothVisited.add((secondLine[0] + i, secondLine[1]))
                        break
            secondLine = (secondLine[0] + distance, secondLine[1])
            ...
            distance = int(instruction[1:])
            for i in range(1, distance + 1):
                if abs(secondLine[0]) + abs(secondLine[1] + i) < tempShortestDistance:
                    if (secondLine[0], secondLine[1] + i) in firstLineVisited:
                        bothVisited.add((secondLine[0], secondLine[1] + i))
                        break
            secondLine = (secondLine[0], secondLine[1] + distance)
    ...
    for coordinate in bothVisited:
        currentDistance = abs(coordinate[0]) + abs(coordinate[1])
...
def Day3Part2(data1, data2):
    firstLine = (0, 0)
    secondLine = (0, 0)
    firstLineVisited = set()
    firstLineVisitedWithSteps = set()
    firstLineSteps = 0
    secondLineSteps = 0
    ...
    for instruction in data1:
        if instruction[0] is "R":
            ...
            for i in range(1, distance + 1): # from 1 to distance
                firstLineSteps += 1
                if (firstLine[0] + i, firstLine[1]) not in firstLineVisited:
                    firstLineVisited.add((firstLine[0] + i, firstLine[1]))
                    firstLineVisitedWithSteps.add((firstLine[0] + i, firstLine[1], firstLineSteps))
            firstLine = (firstLine[0] + distance, firstLine[1])
            ...
            for i in range(1, distance + 1): # from 1 to distance
                firstLineSteps += 1
                if (firstLine[0], firstLine[1] + i) not in firstLineVisited:
                    firstLineVisited.add((firstLine[0], firstLine[1] + i))
                    firstLineVisitedWithSteps.add((firstLine[0], firstLine[1] + i, firstLineSteps))
            firstLine = (firstLine[0], firstLine[1] + distance)
    ...
    for instruction in data2:
        if instruction[0] is "R":
            ...
            for i in range(1, distance + 1): # from 1 to distance
                secondLineSteps += 1
                if (secondLine[0] + i, secondLine[1]) in firstLineVisited:    
                    for item in firstLineVisitedWithSteps:
                        if item[0] == secondLine[0] + i and item[1] == secondLine[1]:
                            if item[2] + secondLineSteps < tempFewestSteps:
                                tempFewestSteps = item[2] + secondLineSteps
                                break
            secondLine = (secondLine[0] + distance, secondLine[1])
            ...
            for i in range(1, distance + 1): # from 1 to distance
                ...
                if (secondLine[0], secondLine[1] + i) in firstLineVisited: 
                    for item in firstLineVisitedWithSteps:
                        if item[0] == secondLine[0] and item[1] == secondLine[1] + i:
                            if item[2] + secondLineSteps < tempFewestSteps:
                                tempFewestSteps = item[2] + secondLineSteps
                                break
            secondLine = (secondLine[0], secondLine[1] + distance)
